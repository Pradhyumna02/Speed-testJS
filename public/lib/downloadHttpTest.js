...
(function () {
     ...
     **/
    function downloadHttpTest(urls, url, type, concurrentRuns, timeout, testLength, movingAverage, callbackComplete, callbackProgress, callbackAbort,
     ...
     * @return error object
     */
    downloadHttpTest.prototype.onTestError = function (result) {
        if (this._running) {
     ...
     * @return abort object
     */
    downloadHttpTest.prototype.onTestAbort = function () {
        if(this._running) {
     ...
     */
    downloadHttpTest.prototype.onTestTimeout = function () {
        if(this._running) {
            if ((Date.now() - this._beginTime) > this.testLength) {
                clearInterval(this.interval);
                if (this.finalResults && this.finalResults.length) {
     ...
     * onComplete method
     */
    downloadHttpTest.prototype.onTestComplete = function (result) {
     ...
     */
    downloadHttpTest.prototype.calculateStats = function () {
     ...
     * Start the test
     */
    downloadHttpTest.prototype.start = function () {
        if (!this._running) {
            return;
        }
        var url;
        if (this.type === 'GET') {
            for (var g = 1; g <= this.concurrentRuns; g++) {
                ...
                var request = new window.xmlHttpRequest('GET', url +  '&r=' + Math.random(), this.timeout, this.onTestComplete.bind(this), this.onTestProgress.bind(this),
...
this.size = this.size/6;
     ...
     */
    downloadHttpTest.prototype.initiateTest = function(){
        this._testIndex = 0;
    ...
    window.downloadHttpTest = downloadHttpTest;
})();
